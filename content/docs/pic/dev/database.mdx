...
| show | int2 | yes | 1 | 显示状态：0=显示，1=隐藏 |
| sort | int2 | yes | 0 | 排序 |
| allow_download | int2 | yes | 1 | 是否显示下载/复制按钮：0=显示，1=隐藏 |
...
| ---- | ---- | ---- | ---- | ---- |
| id | varchar(50) | yes |      | 主键ID（使用cuid生成） |
| name | varchar(200) | yes |      | 版权名称 |
| social_name | varchar(200) | no | | 社交媒体名称 |
| type | varchar(50) | no | | 类型：social/target |
| url | text | yes | | 链接 |
| avatar_url | text | no | | 社交媒体头像地址 |
| detail | text | no | | 社交媒体介绍 |
| default | int2 | yes | 1 | 默认状态：0=默认，1=不默认 |
| show | int2 | yes | 1 | 显示状态：0=显示，1=隐藏 |
| created_at | timestamp | yes | CURRENT_TIMESTAMP | 记录创建时间 |
| updated_at | timestamp | no | | 最后更新时间 |
| del | int2 | yes | 0 | 删除标记：0=正常，1=已删除 |
...
| 字段名 | 类型 | NOT NULL | 默认值 | 说明 |
| ---- | ---- | ---- | ---- | ---- |
| copyrightId | varchar(50) | yes |      | 版权表的id |
| imageId | varchar(50) | yes | | 图片表的id |
